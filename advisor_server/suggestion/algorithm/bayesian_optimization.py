...
class BayesianOptimizationDemo(object):
    ...
    def find_closest_value_in_list(self, the_list, objective_value):
        ...
        for current_value in the_list:
            if abs(current_value - objective_value) < abs(
                    closest_value - objective_value):
    ...
    def get_new_suggestions(self,
        ...
        if len(completed_trials) < random_init_trials:
            ...
            return_trials = randomSearchAlgorithm.get_new_suggestions(
                study_id,
                trials,
                number=random_init_trials - len(completed_trials))
      ...
      """
            ...
            for trial in completed_trials:
                ...
                for param in params:
                    ...
                    if param["type"] == "DOUBLE" or param["type"] == "INTEGER" or param["type"] == "DISCRETE":
                        instance_feature = parameter_values_json[param[
                            "parameterName"]]
                        ...
                        # Example: "ftrl"
                        parameter_value = parameter_values_json[param[
                            "parameterName"]]
                        for feasible_point in feasible_points:
                ...
                # [[], [], ..]: shape=[num_completed_trials/n_example num_features]
                init_points.append(instance_features)
                # [x, x, ...]: shape=[num_completed_trials,]
                init_labels.append(instance_label)

            #import ipdb;ipdb.set_trace()

            # [n_example, n_features]
            train_features = np.asarray(init_points)
            # [n_example]
            train_labels = np.asarray(init_labels)
            current_max_label = train_labels.max()
            ...
            # shape: [100000, n_featues]
            x_tries = np.random.uniform(
                bounds[:, 0], bounds[:, 1], size=(100000, bounds.shape[0]))

            # shape: [100000]
            if acq == 'ucb':
                acquisition_fucntion_values = self._ucb(x_tries, gp, kappa)
            elif acq == 'ei':
                acquisition_fucntion_values = self._ei(x_tries, gp,
                                                       current_max_label, xi)
            elif acq == 'poi':
                acquisition_fucntion_values = self._poi(x_tries, gp,
                                                        current_max_label, xi)
            ...
            for param in params:
                ...
                if param["type"] == "DOUBLE" or param["type"] == "DISCRETE":
                    suggested_parameter_values_json[param[
                        "parameterName"]] = x_max[index]
                    index += 1
                elif param["type"] == "INTEGER":
                    suggested_parameter_values_json[param[
                        "parameterName"]] = int(round(x_max[index]))
                    index += 1
                elif param["type"] == "DISCRETE":
                    feasible_points_string = param["feasiblePoints"]
                    feasible_points = [
